function [ constraintModel ] = relaxedApplyEFluxConstraints( model, expression, varargin)
...
if ~expression.preprocessed
    ...
    reactionExpression(pres) = expression.value(pos(pres));    
end

unconstraintReactions = reactionExpression == -1;
maxFlux = max(reactionExpression(~unconstraintReactions));
reactionExpression(unconstraintReactions) = 1000;

% scale to 1000 instead-1 is too restrictive
reactionExpression(~unconstraintReactions) = reactionExpression(~unconstraintReactions)/maxFlux*1000;
...
if ~softBounds
    ...
    modelTest = model;
    if model.lb(i)<0
        modelTest.lb(i) = -reactionExpression(i);
    end
    modelTest.ub(i) = reactionExpression(i);
    FBA = optimizeCbModel(modelTest, 'max');

    fprintf('Trying to constrain reaction %s, FBA result = %f\n', model.rxns{i}, FBA.f);
